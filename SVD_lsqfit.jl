...
# load model data
Data, nx, ny = read_model_data(which_files=2:27,tsteps=2:60;model_files)

# center the data and compute SVD
...
Id, I_intr = load("data/aerodem_data_indices.jld2","I_marg","I_intr")

# solve the lsqfit problem
UΣ            = U*diagm(Σ)
U_A, Σ_A, V_A = svd(UΣ[Id,:])
D             = transpose(diagm(Σ_A))*diagm(Σ_A) + λ*I
v_rec         = V_A * D^(-1) * transpose(diagm(Σ_A)) * transpose(U_A) * x_data[Id]
